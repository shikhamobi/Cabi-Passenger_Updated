...
final class BitmapCroppingWorkerTask extends AsyncTask<Void, Void, BitmapCroppingWorkerTask.Result> {
     ...
     */
    private final WeakReference<com.cabipassenger.util.cropper.CropImageView> mCropImageViewReference;
     ...
     */
    private final com.cabipassenger.util.cropper.CropImageView.RequestSizeOptions mReqSizeOptions;
    ...
    BitmapCroppingWorkerTask(com.cabipassenger.util.cropper.CropImageView cropImageView, Bitmap bitmap, float[] cropPoints,
                             int degreesRotated, boolean fixAspectRatio, int aspectRatioX, int aspectRatioY,
                             int reqWidth, int reqHeight, boolean flipHorizontally, boolean flipVertically, com.cabipassenger.util.cropper.CropImageView.RequestSizeOptions options,
    ...
    BitmapCroppingWorkerTask(com.cabipassenger.util.cropper.CropImageView cropImageView, Uri uri, float[] cropPoints,
                             ...
                             boolean flipHorizontally, boolean flipVertically, com.cabipassenger.util.cropper.CropImageView.RequestSizeOptions options,
                             Uri saveUri, Bitmap.CompressFormat saveCompressFormat, int saveCompressQuality) {
    ...
    protected Result doInBackground(Void... params) {
        try {
            if (!isCancelled()) {
                ...
                if (mUri != null) {
                    bitmapSampled = com.cabipassenger.util.cropper.BitmapUtils.cropBitmap(mContext, mUri, mCropPoints, mDegreesRotated, mOrgWidth, mOrgHeight,
                            mFixAspectRatio, mAspectRatioX, mAspectRatioY, mReqWidth, mReqHeight, mFlipHorizontally, mFlipVertically);
                } else if (mBitmap != null) {
                    bitmapSampled = com.cabipassenger.util.cropper.BitmapUtils.cropBitmapObjectHandleOOM(mBitmap, mCropPoints, mDegreesRotated, mFixAspectRatio,
                            mAspectRatioX, mAspectRatioY, mFlipHorizontally, mFlipVertically);
                } else {
                    return new Result((Bitmap) null, 1);
                }

                Bitmap bitmap = com.cabipassenger.util.cropper.BitmapUtils.resizeBitmap(bitmapSampled.bitmap, mReqWidth, mReqHeight, mReqSizeOptions);
              //  System.out.println("cameeeeeeedd__"+bitmap);
                if (mSaveUri == null) {
              //      System.out.println("cameeeeeeedd__$*"+bitmap);
                    return new Result(bitmap, bitmapSampled.sampleSize);
                } else {
                    com.cabipassenger.util.cropper.BitmapUtils.writeBitmapToUri(mContext, bitmap, mSaveUri, mSaveCompressFormat, mSaveCompressQuality);
                    if (bitmap != null) {
                        bitmap.recycle();
                    }
                 //   System.out.println("cameeeeeeedd__*"+bitmap);
